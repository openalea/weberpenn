...
from  openalea.plantgl.all import (Vector3, Point3Array, Polyline, Translation,
                                   axisRotation,
                                   Matrix3, Matrix4, Transform4, Index3Array,
                                   Index3, TriangleSet,
...
class Leaf(object):
    ...
    def geom(self):
        ...
        points = Point3Array([Vector3(x / 2., 0, 0), Vector3(0, x / 4., 0),
                              Vector3(0, y - x / 4., 0),
                              Vector3(x / 2., y, 0), Vector3(x, y - x / 4., 0),
                              Vector3(x, x / 4., 0)])
        indices = Index3Array(
            [Index3(0, 1, 5), Index3(1, 2, 5), Index3(2, 3, 4),
             Index3(2, 4, 5)])
...
class Weber_Laws(Weber_Penn):
    ...
    def get_radius(self, order, nb_nodes, length, branching=None):
        ...
        if order == 0:
            ...
            taper = 0.9
            radius = parent_radius * pow(1 - taper * length / parent_len,
                                         ratio_power)
    ...
    def nb_leaves(self, axis_id):
        ...
        leaves_per_branch = leaves * shape_ratio(4,
                                                 offset_child / length_parent)
