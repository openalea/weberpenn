...
def _arbaro2params(symbols):
    ...
    if "0SplitAngle" in symbols:
        n_split_angle.append((float(symbols["0SplitAngle"]), float(symbols["0SplitAngleV"])))
    if "1SplitAngle" in symbols:
        n_split_angle.append((float(symbols["1SplitAngle"]), float(symbols["1SplitAngleV"])))
    if "2SplitAngle" in symbols:
        n_split_angle.append((float(symbols["2SplitAngle"]), float(symbols["2SplitAngleV"])))
    if "3SplitAngle" in symbols:
        n_split_angle.append((float(symbols["3SplitAngle"]), float(symbols["3SplitAngleV"])))

    n_down_angle = []
    if "1DownAngle" in symbols:
        n_down_angle.append((float(symbols["1DownAngle"]), float(symbols["1DownAngleV"])))
    if "2DownAngle" in symbols:
        n_down_angle.append((float(symbols["2DownAngle"]), float(symbols["2DownAngleV"])))
    if "3DownAngle" in symbols:
        n_down_angle.append((float(symbols["3DownAngle"]), float(symbols["3DownAngleV"])))
