...
from openalea.plantgl.all import Viewer, Scene, Shape, Material, Color3, Extrusion,\
                    Polyline, Point2Array, Vector2, angle, AxisRotated,\
                    Translated, Scaled, Vector3, EulerRotated, Vector4
from random import uniform
from math import pi

brown= Material(Color3(120,60,10))
green= Material(Color3(4,69,4),diffuse=0.768116,specular=Color3(116,116,116))
...
class GeomEngine(object):
    ...
    def __init__(self, server, section, position= Vector3(0,0,0), color= brown, leaf= None):
        self.server= server
        self.color= color
    ...
    def leaves( self, color= green ):
        ...
        map( lambda x: self._shapes.append(self.leaf_shape(x,color,leaf)), leaves )
    ...
    def shapes( self, scale, polyline, radius ):
        ...
        if scale == 'polyline':
            ...
            if scale == 'point':
                ...
                return [Shape(pgl.PointSet(points.pointList), self.color)]
            else:
